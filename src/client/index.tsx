/**
 * Browser half of dsh-context-memory: the Context memory page inside Settings.
 *
...
function ContextmemoryPage() {
  const { html, error, reload } = usePanel()
  if (error !== null) {
    return (
      <section>
        <p role="alert">Context memory panel failed to load: {error}</p>
        <button type="button" onClick={reload}>Retry</button>
      </section>
    )
  }
  if (html === null) return <p aria-live="polite">Loading the Context memory panel…</p>
